...
<?php
 ...
 if (isset($_POST['updatebtn'])) {
	       //session_start(); 
		  $id = $_POST['Typeoftrain'];
		  $myVariable = $_POST['Typeoftrain'];
		  echo 'THE name of the list has changed ' . $myVariable;
		  echo ' Th ename of the type of train is  ' .$category[$id]['trainingtype']; 
		  
		  print($_POST['Typeoftrain']);
...
<label for="gender">SELECT TRAINING TYPE</label><br> 
<?php
...
<?php
echo '<select name="Typeoftrain"  >';
foreach ($categories as $category) {
     echo '<option value="' . $category['id'] . '">' . $category['trainingtype'] . '</option>';
}
echo '</select>';
?>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    echo "Selected category ID: " . $_SESSION['trantype'];
